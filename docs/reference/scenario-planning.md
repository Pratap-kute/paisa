...
A comparison needs a known starting cash balance plus income, expense, and
investment-transfer assumptions. Enter a value manually when history cannot
provide one. A zero investment balance is valid.

## Money and return assumptions
...
The chart applies the assumptions you select; it is not a market prediction or
investment advice. Other assets and liabilities stay constant. Scenarios do not
model tax, inflation, debt amortization, goals, retirement, probabilities, or
recommendations, and they are not saved.
