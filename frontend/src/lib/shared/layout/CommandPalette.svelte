<script lang="ts">
  ...
  const staticCommands: CommandItem[] = [
    ...
    // Assets & Investments
    {
      ...
      category: "Navigation",
      keywords: ["stocks", "funds", "portfolio", "assets", "investments", "holdings"],
  ...
  const filteredCommands = $derived.by(() => {
    ...
    if (trimmed && $accountTfIdf?.tf_idf) {
      ...
      for (const acc of matchedAccounts) {
        accountItems.push({
          ...
          category: "Accounts",
          action: () => navigate(`/ledger/transaction?query=${encodeURIComponent(acc)}`),
        });
      }
    }
    ...
    const searchActionItem: CommandItem = {
      ...
      category: "Search",
      action: () => navigate(`/ledger/transaction?query=${encodeURIComponent(query.trim())}`),
    };
...
<BitsDialog.Root bind:open>
  <BitsDialog.Portal>
    ...
    >
      <!-- SEARCH INPUT HEADER -->
      <div class="flex items-center gap-3 border-b border-[var(--paisa-border-subtle)] bg-[var(--paisa-surface-elevated)] px-4 py-3">
      ...
      <!-- FOOTER -->
      <div class="flex items-center justify-between border-t border-[var(--paisa-border-subtle)] bg-[var(--paisa-surface-elevated)] px-4 py-2 text-[0.6875rem] text-[var(--paisa-muted-foreground)]">
        <div class="flex items-center gap-3">
          <span class="flex items-center gap-1">
            <kbd class="rounded border border-[var(--paisa-border-subtle)] bg-[var(--paisa-surface)] px-1 font-mono">↑</kbd>
            <kbd class="rounded border border-[var(--paisa-border-subtle)] bg-[var(--paisa-surface)] px-1 font-mono">↓</kbd>
            <span>Navigate</span>
          </span>
          <span class="flex items-center gap-1">
        ...
        <div class="flex items-center gap-1">
          <span>Shortcuts:</span>
          <kbd class="rounded border border-[var(--paisa-border-subtle)] bg-[var(--paisa-surface)] px-1 font-mono">
            {isMac ? "⌘K" : "Ctrl K"}
          </kbd>
          <span>or</span>
          <kbd class="rounded border border-[var(--paisa-border-subtle)] bg-[var(--paisa-surface)] px-1 font-mono">/</kbd>
