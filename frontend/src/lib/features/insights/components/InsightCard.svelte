...
{#if compact}
  ...
  <a
  href={p.href}
  class="flex flex-col justify-between p-3 rounded-lg bg-surface border border-border-subtle {accent.border} shadow-xs hover:shadow-sm hover:bg-surface-hover transition-all duration-150 group min-w-0"
>
  <div class="flex items-center justify-between gap-1.5 mb-1.5">
      <div class="flex items-center gap-1.5 min-w-0">
        ...
        <span class="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground truncate">
          {p.categoryLabel}
        </span>
      </div>
      {#if p.badgeText}
...
{:else}
  ...
  <a
  href={p.href}
  class="flex flex-col justify-between p-3.5 sm:p-4 rounded-xl bg-surface border border-border-subtle {accent.border} shadow-xs hover:shadow-md transition-all duration-150 group min-w-0"
>
  <div>
      ...
      <div class="space-y-0.5 mb-2.5 min-w-0">
        <h4 class="text-xs sm:text-sm font-semibold text-foreground group-hover:text-primary transition-colors leading-snug">
          {p.title}
        </h4>
        {#if p.description}
          <p class="text-[11px] sm:text-xs text-muted-foreground leading-normal line-clamp-2">
            {p.description}
          </p>
        {/if}
      </div>
      ...
      {#if p.progressPercent != null}
        <div class="mb-2.5">
          <div class="h-1.5 w-full overflow-hidden rounded-full bg-[var(--paisa-border-subtle)]">
            <div
              class="h-full rounded-full transition-all {p.progressTone === 'critical' ? 'bg-negative' : (p.progressTone === 'warning' ? 'bg-warning' : 'bg-positive')}"
      ...
      {#if p.tags && p.tags.length > 0}
        <div class="flex flex-wrap gap-1 mb-2.5">
          {#each p.tags as tag}
            <span class="inline-flex items-center text-[10px] font-medium px-1.5 py-0.5 rounded bg-surface-raised border border-border-subtle text-foreground">
              {tag.label}
            </span>
          {/each}
        </div>
  ...
  <!-- Action Footer -->
  <div
    class="pt-2 mt-1 border-t border-border-subtle flex items-center justify-between text-[11px] font-semibold text-primary">
